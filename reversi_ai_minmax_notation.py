...
def pos_to_square_name(pos):
    square = pos.y*8+pos.x
...
class Game:
  ...
  # undo game
  def game_undo_move(self,position):
  ...
  def proc_com_turn(self):
    while True:
      if self.is_com_turn():
        ...
        history.append(pos_to_square_name(position))
        if self.board.turn == WHITE:
            print("White AI->(x,y)=",position.x,position.y, pos_to_square_name(position))
        else:
            print("Black AI->(x,y)=",position.x,position.y, pos_to_square_name(position))
...
def click_board(event):
  ...
  if game.board.is_movable(position) == False:
    ...
    return
  print("(x,y)=",position.x,position.y, pos_to_square_name(position))
  history.append(pos_to_square_name(position))
...
# 「対局開始」ボタンを設置
button_undo = tkinter.Button(root, \
  text = u"undo", width=15, command=play_undo)
button_undo.place(x = 300, y = 40)
